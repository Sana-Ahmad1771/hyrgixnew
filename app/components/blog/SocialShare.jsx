...
export default function SocialShare({ title, slug }) {
    ...
    const socialLinks = [
        ...
        },
        {
            ...
            color: copied ? 'bg-green-500 text-white' : 'hover:bg-[#F37303] hover:text-white',
                ...
                >
                    ...
                    <AnimatePresence>
                        {showTooltip === social.name && (
                            <motion.span
                                className="absolute left-full ml-3 px-2 py-1 bg-[#171717] text-white text-xs rounded whitespace-nowrap"
                                initial={{ opacity: 0, x: -10 }}
                                animate={{ opacity: 1, x: 0 }}
                                exit={{ opacity: 0, x: -10 }}
                                transition={{ duration: 0.2 }}
                            >
                                {copied && social.name === 'Copy Link' ? 'Copied!' : social.name}
                            </motion.span>
                        )}
                    </AnimatePresence>
                </motion.button>
...
export function MobileSocialShare({ title, slug }) {
    ...
    const socialLinks = [
        { name: 'WhatsApp', url: `https://wa.me/?text=${encodedTitle}%20${encodedUrl}`, color: '#25D366' },
        { name: 'Facebook', url: `https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}`, color: '#1877F2' },
        { name: 'Twitter', url: `https://twitter.com/intent/tweet?url=${encodedUrl}&text=${encodedTitle}`, color: '#000000' },
